...
1. **Event discovery** — The plugin discovers Bukkit/Paper event classes and builds dot-notation keys. When an event fires, it builds an **event context** (key + key/value map from the event).
...
| File | Purpose |
|------|---------|
| `docs.html` | Main user-facing docs: overview, event list by category, placeholders, and regex reference |
...
Docs are **offline-first**: no hosted site or internet required. Open `docs.html` in a browser to browse events by category, see placeholders per event, and use the regex transform reference.
...
- **messages.yaml** — Named message templates with `{placeholder}` and optional `{key|regex:pattern:replacement}`.
