...
public class DocumentationGenerator {
    ...
    private String buildWikiContent(List<EventDefinition> events, List<EventDefinition> baseEvents,
        ...
        builder.append("</div>\n");
        builder.append("<div id=\"placeholder-transformsPanel\" class=\"panel\"><div id=\"placeholder-transforms\" data-anchor=\"placeholder-transforms\">\n");
        builder.append(innerContent(buildPlaceholderTransformsSection()));
    ...
    private String buildDocsContent(List<EventDefinition> events) {
        ...
        builder.append(buildPlaceholderTransformsSection());
    ...
    private String buildPlaceholderTransformsSection() {
        StringBuilder builder = new StringBuilder();
        builder.append("<section id=\"placeholder-transforms\" class=\"doc-section\" data-anchor=\"placeholder-transforms\">\n");
        builder.append("<h1>Placeholder transforms</h1>\n");

        builder.append("<h3>Overview</h3>\n");
        builder.append("<p>Placeholder values can be transformed before substitution. Use <code>{key}</code> for the raw value, or <code>{key|transform}</code> / <code>{key|transform:args}</code> to apply transforms. ");
        builder.append("If the key is redacted, <code>[REDACTED]</code> is substituted and no transforms run.</p>\n");

        builder.append("<h3>How chaining works</h3>\n");
        builder.append("<p>Transforms are applied <strong>one after the other, left to right</strong>. Each transform receives the output of the previous one. ");
        builder.append("Separate transforms with <code>|</code> (use <code>\\|</code> for a literal pipe inside a transform).</p>\n");
        builder.append("<p><strong>Example:</strong> <code>{key|trim|lower|truncate:10}</code> — first trims whitespace, then lowercases, then truncates to 10 characters.</p>\n");

        builder.append("<h3>Transform reference</h3>\n");
        builder.append("<table class=\"predicates-table\"><thead><tr><th>Transform</th><th>Args</th><th>Description</th></tr></thead><tbody>\n");
        builder.append("<tr><td><code>trim</code></td><td>—</td><td>Remove leading and trailing whitespace</td></tr>\n");
        builder.append("<tr><td><code>lower</code></td><td>—</td><td>Convert to lowercase</td></tr>\n");
        builder.append("<tr><td><code>upper</code></td><td>—</td><td>Convert to uppercase</td></tr>\n");
        builder.append("<tr><td><code>default</code></td><td>fallback</td><td>Use fallback when value is empty or missing</td></tr>\n");
        builder.append("<tr><td><code>truncate</code></td><td>maxLength</td><td>Limit length to maxLength characters</td></tr>\n");
        builder.append("<tr><td><code>replace</code></td><td>literal:replacement</td><td>Replace literal string (no regex)</td></tr>\n");
        builder.append("<tr><td><code>last-path-segment</code></td><td>—</td><td>Last segment of a path (e.g. world from path/to/world)</td></tr>\n");
        builder.append("<tr><td><code>first-path-segment</code></td><td>—</td><td>First segment of a path</td></tr>\n");
        builder.append("<tr><td><code>map</code></td><td>key1:value1:key2:value2:...</td><td>Exact match: if value equals key1, use value1; etc.</td></tr>\n");
        builder.append("<tr><td><code>regex</code></td><td>pattern:replacement</td><td>Java regex replace; $1, $2 for capture groups; $$ for literal $</td></tr>\n");
        builder.append("</tbody></table>\n");
        builder.append("<p>Use <code>\\:</code> for literal colon in args (e.g. <code>map:foo\\:bar:baz</code>).</p>\n");

        builder.append("<h3>Examples: simple to complex</h3>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Simple — single transform</div>\n");
        builder.append("<pre>{player.name|lower}</pre>\n");
        builder.append("<p>For <code>player.name</code> = <code>Steve</code> → <code>steve</code></p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Default for missing values</div>\n");
        builder.append("<pre>{optional.field|default:Unknown}</pre>\n");
        builder.append("<p>If <code>optional.field</code> is empty or missing → <code>Unknown</code></p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Map boolean to label</div>\n");
        builder.append("<pre>{world.hardcore|map:true:hardcore:false:normal}</pre>\n");
        builder.append("<p>For <code>world.hardcore</code> = <code>true</code> → <code>hardcore</code>; for <code>false</code> → <code>normal</code></p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Extract last path segment</div>\n");
        builder.append("<pre>**{world.name|last-path-segment}** has been loaded</pre>\n");
        builder.append("<p>For <code>world.name</code> = <code>2026/02/hardcore-26/world</code> → <code>**world** has been loaded</code></p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Chained — trim, normalize, truncate</div>\n");
        builder.append("<pre>{chat.raw|trim|lower|truncate:50}</pre>\n");
        builder.append("<p>Trims whitespace → lowercases → truncates to 50 chars. Each step uses the output of the previous.</p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Chained — path segment then uppercase</div>\n");
        builder.append("<pre>{world.name|last-path-segment|upper}</pre>\n");
        builder.append("<p>For <code>2026/02/hardcore-26/world</code> → <code>world</code> → <code>WORLD</code></p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Regex — extract second-to-last path segment</div>\n");
        builder.append("<pre>**{world.name|regex:.*/([^/]+)/[^/]+$:$1}** has been loaded</pre>\n");
        builder.append("<p>For <code>world.name</code> = <code>2026/02/hardcore-26/world</code> → <code>**hardcore-26** has been loaded</code></p>\n");
        builder.append("</div>\n");
        builder.append("<div class=\"example-block\">\n");
        builder.append("<div class=\"example-title\">Chained regex — map true/false</div>\n");
        builder.append("<pre>{key|regex:^true$:hardcore|regex:^false$:normal}</pre>\n");
        builder.append("<p>First regex: if value is <code>true</code> → <code>hardcore</code>. Second regex: if value is <code>false</code> → <code>normal</code>.</p>\n");
        builder.append("</div>\n");

        builder.append("<h3>Invalid or unknown transforms</h3>\n");
        builder.append("<p>If the transform name is unknown or args are invalid, the value passes through unchanged. A one-time warning is logged per placeholder key on error.</p>\n");

        builder.append("<h3>Security</h3>\n");
        builder.append("<p>Regex runs with a 300 ms timeout. On timeout or exception, the resolver uses the untransformed value and logs a one-time warning.</p>\n");
